...
for x in tqdm(to_process, total=len(to_process)):
    ...
    for i, mention in enumerate(
        ...
        if mention.get("type") == "person":
            ...
            actualization = URIRef(
                f"{text_passage}".replace("passage", "actualization")
            )
            g.add((actualization, RDF.type, INT["INT2_ActualizationOfFeature"]))
            g.add(
                (
                    actualization,
                    RDFS.label,
                    Literal(f"Actualization on {xml_id}", lang="en"),
                )
            )
            g.add((text_passage, INT["R18_shows_actualization"], actualization))
            g.add((actualization, INT["R18_actualization_found_on"], text_passage))

            reference = URIRef(f"{text_passage}".replace("passage", "reference"))
            g.add((reference, RDF.type, INT["INT18_Reference"]))
            g.add((reference, RDFS.label, Literal(f"Reference on {xml_id}", lang="en")))
            g.add((reference, INT["R17_feature_actualized_in"], actualization))
            g.add((actualization, INT["R17_actualizes_feature"], reference))
            g.add((reference, CIDOC["P67_refers_to"], person_uri))
